...
class db_connection{	
	...
	//	$sql = "CREATE DATABASE hackathon";
		...
		if($result->num_rows > 0){		
			while($row=$result->fetch_assoc()){
				$this->personList[]=new Person($row["personID"],$row["firstName"],$row["lastName"],$row["phone"],$row["latitude"],
	...
	function addPerson($pN,$f,$l,$p,$lat,$lng,$i){
		$this->personList[]=new Person($pN,$f,$l,$p,$lat,$lng,$i);
		$sql = "INSERT INTO Person (personID,firstName,lastName,phone,latitude,longitud,interest)
		VALUES ('$pN','$f','$l','$p','$lat','$lng','$i')";
		if ($this->connection->query($sql) === False) {
			echo "Error: " . $sql . "<br>" . $this->connection->error;
		}
		$this->connection->close();
